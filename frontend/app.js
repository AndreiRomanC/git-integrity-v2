...
function gitState(entry) {
  ...
  // Submodule-status-labels report: gated on !submodule_is_dirty, not on
  // submodule_has_unpushed_commits — an "M" here can only ever mean a
  // gitlink version bump (staged, pushed or not) once the submodule's own
  // working tree/index is clean, regardless of whether that new commit has
  // reached the submodule's own remote yet. Gating on "still unpushed"
  // instead used to make this badge fall back to a plain "Modified" —
  // indistinguishable from an ordinary dirty file — the moment the
  // submodule was safely pushed but the project hadn't committed the new
  // pointer yet, which is exactly backwards: that's the *safer* of the two
  // states, not a less notable one.
  if (entry.kind === 'submodule' && entry.status === 'M' && !entry.submodule_is_dirty) {
...
function renderEntryDetails(entry) {
  ...
  refs.details.innerHTML = `<div class="entry-details"><div class="entry-preview ${esc(entry.kind)}">${entry.kind === 'submodule' ? '◇' : entry.kind === 'folder' ? '▰' : '▤'}</div>
    ...
    ${entry.status || !entry.tracked ? (() => {
      // Submodule-status-labels report: three genuinely different states
      // used to collapse into two labels. "M" + submodule_push_status (its
      // own commit not pushed yet) and "M" + clean-but-pushed (staged, safe,
      // just not committed in the project yet) both need to read as a
      // version change, not as an ordinary edit — only a dirty submodule
      // working tree (submodule_is_dirty) is genuinely "Modified locally".
      const pending = entry.kind === 'submodule' && entry.submodule_push_status;
      const pushedNotCommitted = entry.kind === 'submodule' && !pending && entry.status === 'M' && !entry.submodule_is_dirty;
      const title = pending ? 'New version locally (not pushed yet)' : pushedNotCommitted ? 'New version (pushed, not committed in the project yet)' : entry.tracked ? 'Modified locally' : 'New local file';
      const detail = pending ? 'Committed here, on this machine — not sent to the submodule\'s server yet.' : pushedNotCommitted ? 'Already pushed to the submodule\'s own remote — stage/commit the project to record this new version.' : 'This item differs from the committed repository state.';
      return `<div class="local-change-banner"><i></i><div><strong>${title}</strong><span>${detail}</span></div></div>`;
    })() : ''}
